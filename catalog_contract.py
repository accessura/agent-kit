...
def normalize_signal_schema(signal_schema: dict[str, str]) -> dict[str, str]:
    ...
    try:
        encoded = json.dumps(signal_schema, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
...
def assert_catalog_parity(catalog: dict[str, Any]) -> None:
    ...
    if signal_contract.get("requiredForBiddablePacks") is not True:
        errors.append("signalContract.requiredForBiddablePacks drift")
    if signal_contract.get("schemaField") != "signal_schema":
        errors.append("signalContract.schemaField drift")
    if signal_contract.get("typeField") != "signal_type":
        errors.append("signalContract.typeField drift")
    if signal_contract.get("separateFrom") != "fields (Pack delivery/container metadata)":
        errors.append("signalContract fields/schema separation drift")
    operations = catalog.get("operations", [])
    publish = next((operation for operation in operations if operation.get("id") == "publish.pack"), None)
    if publish is None:
